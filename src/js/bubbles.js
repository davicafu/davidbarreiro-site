...
function renderBubbles(controller) {
  const { el, state } = controller;
  el.innerHTML = '';

  const w = el.clientWidth || 680;
  const isMobile = w < 640;
  const h = isMobile ? Math.max(430, Math.round(w * 0.96)) : Math.max(560, Math.round(w * 0.9));
  const interactionDuration = reducedMotion ? 0 : 350;

  const svg = select(el).append('svg').attr('viewBox', [0, 0, w, h]).attr('class', 'overflow-hidden');
  ...
  const isLogoAsset = (value) => {
    ...
    return (
      /^\.?\/?assets\/logos\/.+\.(svg|png|webp|avif)$/i.test(source) ||
      /^\.\/assets\/logos\/.+$/i.test(source)
    );
  };
  const fitLabel = (d) => {
  ...
  const groupBubble = groupLayer
    .selectAll('.group-bubble')
    .data(groupsData)
    .join('g')
    .attr('class', 'group-bubble cursor-pointer')
  ...
  groupBubble.on('mousemove', (event, d) => {
    ...
    showTip(
      event,
      `<strong>${d.name}</strong><br><small>${d.level}</small><br><small>${count} ${t(appState.locale, 'bubbles_click_filter', 'skills · click to filter')}</small>`
    );
  });
  groupBubble
    .append('circle')
    ...
    .delay((d, i) => i * 120)
    .ease(easeElasticOut.amplitude(0.8).period(0.5))
    .attr('r', (d) => d.r);
  ...
  const node = nodeLayer
    ...
    .on('mousemove', (event, d) => {
      ...
      showTip(
        event,
        `<small>${d.group}</small>${meta.level ? `<p class="mt-1"><small>${t(appState.locale, 'level_label', 'Level')}: ${meta.level}</small></p>` : ''}${meta.certifications?.length ? `<p class="mt-2"><small>${t(appState.locale, 'cert_label', 'Certifications')}: ${renderCertsInline(meta.certifications)}</small></p>` : ''}${meta.notes ? `<p class="mt-2 text-slate-300">${meta.notes}</p>` : ''}`
      );
    })
    .on('mouseleave', hideTip);
  ...
  node
    .append('circle')
    .attr('class', 'bubble')
    .attr('r', (d) => (reducedMotion ? d.r : 1))
    .attr('fill', (d) => bubbleFill(d.color))
    .attr('opacity', 0.9)
    .attr('stroke', 'rgba(255,255,255,.42)')
    .attr('stroke-width', 1);
  node
    .filter((d) => {
      const logos = getLogoList(d.meta);
  ...
  node
    ...
    .duration(reducedMotion ? 0 : 900)
    .delay((d, i) => i * 35)
    .ease(easeElasticOut.amplitude(0.8).period(0.45))
    .attr('r', (d) => d.r);
  ...
  const toTransform = (cx, cy, width) => {
    const view = clampView(cx, cy, width);
    return zoomIdentity.translate(w / 2 - view.cx * view.k, h / 2 - view.cy * view.k).scale(view.k);
  };
  ...
  const zoomBehavior = zoom()
    ...
    .wheelDelta((event) => {
      ...
      return (
        -event.deltaY *
        (event.deltaMode === 1 ? 0.028 : event.deltaMode ? 0.55 : base) *
        (event.ctrlKey ? 6 : 1)
      );
    })
    .on('zoom', (event) => {
  ...
  if (isMobile) {
    svg.style('touch-action', 'manipulation').on('touchstart', (event) => {
  ...
  const minX = min(nodesForBounds, (d) => d.x - d.r) ?? 0;
  const maxX = max(nodesForBounds, (d) => d.x + d.r) ?? w;
  const minY = min(nodesForBounds, (d) => d.y - d.r) ?? 0;
  const maxY = max(nodesForBounds, (d) => d.y + d.r) ?? h;
  ...
  const zoomTo = (target, animated = true) => {
    const transform = toTransform(target[0], target[1], target[2]);
    ...
    svg
      ...
      .duration(720)
      .ease(easeSinInOut)
      .call(zoomBehavior.transform, transform, [w / 2, h / 2]);
  };

  const zoomToGroup = (groupName, animated = true) => {
    const group = groupsData.find((item) => item.name === groupName);
    if (!group) return;
    const maxZoomWidth = w / maxScale;
    const size = Math.max(maxZoomWidth, group.r * 2.1 + (isMobile ? 42 : 56));
    zoomTo([group.x, group.y, size], animated);
  };
    ...
    document.querySelectorAll('#skill-keywords .keyword-chip').forEach((chip) => {
      chip.addEventListener('click', (event) => {
        ...
        const targetNode = nodes.find(
          (nodeItem) => nodeItem.group === targetGroup && nodeItem.name === targetKeyword
        );
  ...
  function selectGroup(groupName, animated = true) {
    const group = appState.skillGroups.find((item) => item.name === groupName);
    if (!group) return;
    hideInitialTip();
    ...
    zoomToGroup(groupName, animated);
    animate(node, animated ? interactionDuration : 0).attr('opacity', (d) =>
      d.group === groupName ? 1 : 0.05
    );
    animate(node.select('circle'), animated ? interactionDuration : 0)
      .attr('stroke-width', (d) => (d.group === groupName ? 3 : 1))
      .attr('r', (d) => (d.group === groupName ? d.r + 4 : d.r * 0.92));
    animate(groupBubble.select('circle'), animated ? interactionDuration : 0)
      .attr('opacity', (d) => (d.name === groupName ? 0.26 : 0.06))
      .attr('stroke-width', (d) => (d.name === groupName ? 3 : 1.5));
  }

  function selectSkill(skill, animated = true) {
    hideInitialTip();
    state.selectedGroup = skill.group;
    state.selectedSkill = skill.name;
    setActiveLegend(skill.group);
    renderKeywordsPanel(
      appState.skillGroups.find((group) => group.name === skill.group),
      skill.name
    );
    bindKeywordChipEvents();
    highlightKeywordChip(skill.group, skill.name);
    zoomToGroup(skill.group, animated);
    animate(node, animated ? interactionDuration : 0).attr('opacity', (d) =>
      d.name === skill.name ? 1 : d.group === skill.group ? 0.22 : 0.05
    );
    animate(node.select('circle'), animated ? interactionDuration : 0)
      .attr('stroke-width', (d) => (d.name === skill.name ? 4 : 1))
      .attr('r', (d) => (d.name === skill.name ? d.r + 8 : d.r));
    animate(groupBubble.select('circle'), animated ? interactionDuration : 0)
      .attr('opacity', (d) => (d.name === skill.group ? 0.24 : 0.06))
      .attr('stroke-width', (d) => (d.name === skill.group ? 3 : 1.5));
  }

  function resetSelection(animated = true) {
    state.selectedGroup = '';
    state.selectedSkill = '';
    clearActiveLegend();
    renderKeywordsPanel(null);
    zoomTo([fitCx, fitCy, w / initialScale], animated);
    animate(node, animated ? interactionDuration : 0).attr('opacity', 1);
    animate(node.select('circle'), animated ? interactionDuration : 0)
      .attr('stroke-width', 1)
      .attr('r', (d) => d.r);
    animate(groupBubble.select('circle'), animated ? interactionDuration : 0)
  ...
  controller.actions = { selectGroup };

  groupBubble.on('click', function (event, d) {
    event.stopPropagation();
    selectGroup(d.name);
  });
  ...
  document.querySelectorAll('.skill-filter').forEach((button) => {
    button.onclick = () => controller.actions.selectGroup(button.dataset.group);
  });
  svg.on('click', () => {
    hideInitialTip();
    resetSelection();
  ...
  if (state.selectedSkill) {
    const selectedNode = nodes.find(
      (nodeItem) => nodeItem.group === state.selectedGroup && nodeItem.name === state.selectedSkill
    );
    if (selectedNode) {
      selectSkill(selectedNode, false);
      return;
    }
    state.selectedSkill = '';
  }
  if (state.selectedGroup) {
    selectGroup(state.selectedGroup, false);
    return;
  }
...
function bubbles() {
  ...
  const existing = bubbleControllers.get(el);
  if (existing) {
    return existing;
  }
  ...
  const controller = {
    ...
    actions: { selectGroup() {} },
    refresh({ force = false } = {}) {
      if (!this.el?.isConnected) return this;
      const nextWidth = this.el.clientWidth || 680;
      if (this.state.mounted && !force && this.state.lastWidth === nextWidth) {
        return this;
      }
      this.state.lastWidth = nextWidth;
  ...
  bubbleControllers.set(el, controller);
