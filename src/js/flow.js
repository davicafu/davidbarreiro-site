...
function renderFlow(controller) {
  ...
  const interactionDuration = reducedMotion ? 0 : 220;
  ...
  let runningY = topPad;
  const layout = appState.flow.map((step, index) => {
  ...
  const links = pairs(layout).map(([a, b], index) => ({
    id: `link-${index}`,
    from: a.i,
    to: b.i,
    x1: a.x + a.width / 2,
  ...
  linkEl
    ...
    .delay((d, index) => index * 120)
    .ease(easeCubicOut)
    .attr('x2', (d) => d.x2)
    .attr('y2', (d) => d.y2);

  const card = layerCards
    .selectAll('.flow-card')
    .data(layout)
    .join('g')
  ...
  card
    ...
    .selectAll('tspan')
    .data((d) => d.detailLines.map((line, index) => ({ line, index })))
    .join('tspan')
    .attr('x', isMobile ? 38 : 44)
    .attr('dy', (item) => (item.index === 0 ? 0 : detailLineH))
    .text((item) => item.line);
  ...
  card
    ...
    .delay((d, index) => index * 90)
    .ease(easeCubicOut)
    .attr('opacity', 1)
    .attrTween('transform', function (d) {
      const interpolatedY = interpolateNumber(18, 0);
      return (value) => `translate(${d.x},${d.y + interpolatedY(value)})`;
    });
  ...
  const detailTitle = detailCard
    ...
    .attr('font-weight', 900);
  const detailImpact = detailCard
    .append('text')
    .attr('x', 16)
    .attr('y', 46)
    .attr('fill', '#94a3b8')
    .attr('font-size', isMobile ? 10 : 11);
  const detailTech = detailCard
    .append('text')
    .attr('x', 16)
    .attr('fill', '#cbd5e1')
    .attr('font-size', isMobile ? 10 : 11)
  ...
  const triggerFlowCardFx = (index) => {
    ...
    const sparks = target.append('g').attr('class', 'flow-card-fx-overlay').style('pointer-events', 'none');
    const cornerData = [
    ...
    sparks
      ...
      .attr('r', 0.2)
      .attr('opacity', 0)
      .on('end', function (_, index) {
        if (index === cornerData.length - 1) sparks.remove();
      });
  };
  ...
  const setFlowViewBox = (expandedIndex) => {
    ...
    const shiftedLastBottom = layout.length
      ? layout[layout.length - 1].y +
        getCardOffsetY(layout[layout.length - 1].i, expandedIndex) +
        layout[layout.length - 1].height
      : baseChartBottom;
  ...
  function applyFlowLayout(expandedIndex = -1, animated = true) {
    ...
    const cardMotion = duration > 0 ? card.transition().duration(duration).ease(easeCubicOut) : card;
    const linkMotion = duration > 0 ? linkEl.transition().duration(duration).ease(easeCubicOut) : linkEl;

    cardMotion.attr('transform', (d) => `translate(${d.x},${d.y + getCardOffsetY(d.i, expandedIndex)})`);
    ...
    if (duration > 0) {
      detailCard.attr('transform', `translate(${step.x},${targetY + 14}) scale(.992)`).attr('opacity', 0);
      detailCard
        .transition()
        .duration(duration)
        .ease(easeCubicOut)
  ...
  function setFlowFocus(index = -1, animated = true) {
    animate(card, animated ? interactionDuration : 0).attr('opacity', (d) =>
      index < 0 ? 1 : d.i === index ? 1 : 0.23
    );
    animate(card.select('rect:nth-child(1)'), animated ? interactionDuration : 0)
      .attr('stroke', (d) => (d.i === index ? 'rgba(103,232,249,.8)' : 'rgba(148,163,184,.22)'))
      .attr('stroke-width', (d) => (d.i === index ? 2.1 : 1.3));
    animate(linkEl, animated ? interactionDuration : 0)
      .attr('opacity', (d) => (index < 0 ? 0.88 : d.from === index || d.to === index ? 1 : 0.12))
      .attr('stroke', (d) =>
        d.from === index || d.to === index ? 'rgba(34,211,238,.95)' : 'rgba(103,232,249,.35)'
      )
      .attr('stroke-width', (d) => (d.from === index || d.to === index ? 2.8 : 1.8));

    if (index < 0 || !layout[index]) return;
    ...
    const impactLines = wrapTextLines(
      step.impact,
      Math.max(26, Math.floor((cardW - 34) / (isMobile ? 6 : 6.6)))
    );
    detailImpact
      .selectAll('tspan')
      .data(impactLines.map((line, index) => ({ line, index })))
      .join('tspan')
      .attr('x', 16)
      .attr('dy', (item) => (item.index === 0 ? 0 : isMobile ? 13 : 14))
      .text((item) => item.line);
    ...
    detailTech
      ...
      .data(
        wrapTextLines(
          `Tech: ${step.tech.join(' · ')}`,
          Math.max(24, Math.floor((cardW - 34) / (isMobile ? 6 : 6.6)))
        ).map((line, index) => ({
          line,
          index
        }))
  ...
  let pinnedFlowIndex = Number.isInteger(state.pinnedFlowIndex) ? state.pinnedFlowIndex : -1;

  const getVisibleFlowIndex = () => (pinnedFlowIndex >= 0 ? pinnedFlowIndex : hoverFlowIndex);

  const syncFlowState = (animated = true) => {
    activeFlowIndex = getVisibleFlowIndex();
    setFlowFocus(activeFlowIndex, animated);
    applyFlowLayout(activeFlowIndex, animated);
  };

  card
    .on('mouseenter', (event, d) => {
      if (isMobile || pinnedFlowIndex >= 0) return;
      hoverFlowIndex = d.i;
      syncFlowState();
    ...
    .on('click', (event, d) => {
      ...
      state.pinnedFlowIndex = pinnedFlowIndex;
  ...
  if (flowTipTarget) {
    ...
    notePanel.className =
      'section-tip-contrast mt-2 rounded-2xl border border-slate-800 bg-slate-950/78 p-3 text-xs text-slate-300';
    notePanel.innerHTML = flowTipHtml;
    el.appendChild(notePanel);
  }

  if (pinnedFlowIndex >= layout.length) {
    state.pinnedFlowIndex = -1;
    pinnedFlowIndex = -1;
  }
  syncFlowState(false);
...
function dataFlow() {
  ...
  const existing = flowControllers.get(el);
  if (existing) {
    return existing;
  }
  ...
  const controller = {
    ...
    refresh({ force = false } = {}) {
      ...
      const nextWidth = this.el.clientWidth || 980;
  ...
  flowControllers.set(el, controller);
