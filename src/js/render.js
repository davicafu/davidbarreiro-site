...
function mapResumeToViewModel(data) {
  ...
  const educationItems = education.map((entry) => ({
    ...
    highlights: [],
    tech:
      Array.isArray(entry.tech) && entry.tech.length
...
function renderHeader(basics) {
  ...
  if (kicker)
    kicker.textContent = `${safeText(basics.label, 'Senior Software Engineer | Distributed Systems · Data Platforms · Streaming')}`;
  if (nameEl) nameEl.textContent = firstName;
  if (surnameEl) surnameEl.textContent = surname;
  if (summary)
    summary.textContent = safeText(
      basics.summary,
      'I build real-time data platforms and deliver pragmatic solutions for complex systems.'
    );
  ...
  if (profileImage) {
    ...
    const githubProfile =
      profiles.find((p) => safeText(p.network).toLowerCase().includes('github'))?.url || '';
    ...
    profileImage.onerror = function () {
      ...
      this.onerror = null;
      this.src = '/favicon/favicon.ico';
      this.className = 'w-28 h-28 rounded-[1.7rem] object-contain p-2 bg-white/5 mb-7 border border-white/20';
    };
    const sources = [primaryImage, githubAvatar, '/favicon/favicon.ico'].filter(Boolean);
    profileImage.src = sources[0];
  }
  ...
  if (contactSectionMail) {
    contactSectionMail.href = `mailto:${email}`;
    contactSectionMail.textContent = email;
  }
...
function renderMetrics(_basics = {}, _meta = {}) {
  ...
  const advancedCoreSkills = appState.skillGroups
    ...
    .filter((keyword) => isCoreAdvanced(keyword.level)).length;
...
function renderProjects(portfolio) {
  ...
  const normalizeTag = (value) =>
    safeText(typeof value === 'string' ? value : value?.text || value?.name || '', '').trim();
  const allTags = Array.from(
    new Set(list.flatMap((project) => (project.highlights || []).map(normalizeTag).filter(Boolean)))
  ).slice(0, 28);
  ...
  [...selectedTags].forEach((tag) => {
    if (!allTags.includes(tag)) selectedTags.delete(tag);
  });
...
function renderLegend() {
  ...
  root.innerHTML = appState.skillGroups
    .map(
      (group) =>
        `<button type=\"button\" class=\"skill-filter inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-slate-800 text-sm text-slate-300 hover:border-cyan-300 transition\" data-group=\"${group.name}\"><span class=\"w-2.5 h-2.5 rounded-full\" style=\"background:${group.color}\"></span>${group.name}</button>`
    )
    .join('');
}

function setActiveLegend(groupName) {
  document.querySelectorAll('.skill-filter').forEach((button) => {
    button.classList.toggle('active', button.dataset.group === groupName);
  });
}
function clearActiveLegend() {
  document.querySelectorAll('.skill-filter').forEach((button) => {
